...
void HVecB::HipVec(double L1,double L2,double A,double B, double L, double M, double K, double* vx, double* vy, double* vz){
...
float HVecB::floatmap(float x, float in_min, float in_max, float out_min, float out_max){
      return (x - in_min) * (out_max - out_min) / (in_max - in_min) + out_min;
}

void HVecB::PTMRead(double P1,double* A1,double map){
    float angle1 = 0;
...
double NerotB::ABV(double x, double y, double z, double w) {
    double B = ((acos(((x*y)+(z*w))/((sqrt((x*x)+(y*y)))*(sqrt((z*z)+(w*w)))))));
    // std::cout << B;
    return B;
}

double NerotB::QuadXY(double x, double y)
{
    double B = 0;
...
//return B; 
//}
double JoyTestB::joyread( int maxmin, int pin, int angle){
    HVecB o;
...
double NerotB::ABVS(double x,double y,double z,double w){
    double B = 0;
...
double NerotB::AngleXY(double x, double y){
    NerotB O;
...
void EtoQB(double ze,double ye,double xe, double* x, double* y, double* z, double* w){
...
void MechaLibB::EtoQ(double X1,double Y1,double Z1, double* X, double* Y, double* Z, double* W){
...
Quat MechaLibB::getQuat(int16_t k){
    MechaLibB mlb;
    NerotB rot;
    HVecB vec;
    Quat Q;
    int16_t rX2, rY2, rZ2;
    BMI1.getRotation(&rX2, &rY2, &rZ2);
    JoyTestB J;
    double x1, y1, z1, w1;
    x1 = y1 = z1 = w1 = 0;
    double P1,P2,P3,P4;
